...
class SFSHandler
{
	...
	{
		log("Logged.");
		sfs.send(new com.smartfoxserver.v2.requests.SpectatorToPlayerRequest());
		play();
	}
	...
	private function onConnection(e:SFSEvent):Void 
	{
		...
		#end
		{
			...
			#if html5
			sfs.send(new LoginRequest(null, null, null, "BasicExamples"));
			#else
			sfs.send(new LoginRequest(null, null, "BasicExamples"));
			#end
		}else{
